// plugins/reactions.js - ESM Version
...
// Helper function to send reaction GIFs
async function sendReaction(conn, mek, m, { reply }, action, emoji, messageTemplate) {
    try {
        ...
        let message;
        if (mentionedUser) {
            message = messageTemplate.withMention(sender, mentionedUser);
        } else if (isGroup) {
            message = messageTemplate.withGroup(sender);
        } else {
        ...
        reply(`❌ *Error in .${action} command:*\n\`\`\`${error.message}\`\`\``);
    }
}

// Helper templates
const templates = {
    withMention: (sender, target) => `${sender} ${target.split("@")[0]}`,
    withGroup: (sender) => `${sender}`
};
